...
form.addEventListener('submit', async (event) => {
  ...
  if (!config?.url || config.url.includes('YOUR-PROJECT') || !config?.anonKey || config.anonKey.includes('YOUR-ANON')) {
    showMessage('The application form is ready, but Supabase has not been connected yet. Add your Project URL and anon key in supabase-config.js.', 'error');
    return;
  }
  ...
  if (error) return showMessage('We could not submit your application. Please try again or contact live@relvia.online.', 'error');
  form.reset();
  showMessage('Application received. The Relvia Live team will be in touch if there is a fit.', 'success');
});
